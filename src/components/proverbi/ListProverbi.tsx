...
export default function ListProverbi({ type, setCount, isOwner }: Readonly<ListProps>) {
  ...
  const renderEmptyState = () => {
    if (type === "accepted") {
      return renderEmptyStateAccepted();
    }
    if (type === "salvati") {
      return renderEmptyStateSalvati();
    }
    if (type === "admin") {
      return renderEmptyStateAdmin();
    }
    return null;
  };
