...
export default function ProverbioBody() {
    ...
    useEffect(() => {
        async function loadProverbioFromSEO() {
            const result = await getProverbioFromSEO(pathname.split("/").findLast(Boolean) || "");
            if (result) {
                setProverbioObj(result);
                setMsg("Guarda%20il%20proverbio%20di%20" + result.username + "%20su%20Proverby%21%20https%3A%2F%2Fwww.proverby.it%2Fproverbio%2F" + pathname.split("/").findLast(Boolean))
                setIsLoading(false);
            }
        }
    ...
    return (
        <div className="flex flex-col gap-2.5">
                        ...
                        <div className="flex justify-between gap-2.5 w-full">
                            ...
                            <div className="hidden md:flex flex-col items-start gap-2.5">
                                ...
                                <div className="flex items-center gap-5 text-[1.8rem]">
                                    ...
                                    <a href={`https://www.facebook.com/sharer/sharer.php?u=https%3A%2F%2Fwww.proverby.it%2Fproverbio%2F${pathname.split("/").findLast(Boolean)}`} target="_blank" aria-label="Share on facebook" className="opacity-50 hover:opacity-100 hover:duration-300"><BiLogoFacebookCircle /></a>
                                </div>
                            </div>
                        </div>
                                ...
                                <div className="flex flex-col gap-2.5 md:gap-[5px] mt-3 rounded-(--border-radius) bg-(--contrast-007) w-full p-[15px_20px] border-l-5 border-(--contrast-01)">
                                    {
                                        proverbioObj.esempi.map((esempio, i) => (
                                            <div className="flex items-center gap-2.5" key={`${i}-${esempio}`}><div><AiOutlineDoubleLeft className="text-[1.2rem] opacity-20" /></div><p>{esempio}</p><div><AiOutlineDoubleRight className="text-[1.2rem] opacity-20" /></div></div>
                                        ))
                                    }
                                </div>
                            </div>
            ...
            <div className="relative w-full border-solid border border-(--contrast-01) rounded-(--border-radius) p-[20px_20px_100px_20px] text-center  mt-0 md:mt-[50px] mb-[75px]">
                ...
                <button className="absolute left-[50%] top-full transform-[translate(-50%,-50%)] flex items-center justify-center w-[140px] h-[140px] bg-(--primary) rounded-full shadow-[0_5px_0_var(--primary-dark)] active:shadow-[0_0_0_var(--primary-dark)] active:transform-[translate(-50%,calc(-50%+5px))] duration-300 cursor-pointer" onClick={handleRandom}>
